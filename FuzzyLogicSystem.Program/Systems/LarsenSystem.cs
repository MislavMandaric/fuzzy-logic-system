...
{
    public class LarsenSystem : BaseInferenceSystem
    {
        public LarsenSystem(IFuzzyfier fuzzyfier, IDefuzzyfier defuzzyfier)
            : base(fuzzyfier, defuzzyfier)
        {
            this.sNorm = new ZadehS();
            this.tNorm = new AlgebraT();
            this.not = new ZadehNot();
            this.implication = new Product();
            this.composition = new MaxProduct();
        }
        ...
        protected override IFuzzySet GetAngle(params string[][] variables)
        {
            ...
            foreach (var rule in angleRules)
            {
                ...
                double min = values.Aggregate((current, next) => current * next);
